...
protected:
    ...
    void SetUp() override
    {
        car_params.mass = 269.6;                     // kg
        car_params.front_axle_distance = 1.20;       // m
        car_params.rear_axle_distance = 1.20;        // m
        car_params.cornering_stiffness_front = 127;  // N/rad
        car_params.cornering_stiffness_rear = 127;   // N/rad
        car_params.max_lateral_acceleration = 1.48 * 9.81; // m/s^2
    }
};
...
TEST_F(SteadyStateYawRateReferenceGeneratorTest, ComputesYawRate) {
    ...
    double expected_yaw_rate =
        state.velocity_x /
        (L * (1 + k * state.velocity_x * state.velocity_x)) *
        command.steering;
...
/**
 * @brief Test: Increasing steering increases yaw rate
 */
...
TEST_F(SteadyStateYawRateReferenceGeneratorTest, OverrideUndersteerCoefficient) {
    ...
    double expected_yaw_rate =
        state.velocity_x /
        (L * (1 + custom_k * state.velocity_x * state.velocity_x)) *
        command.steering;
